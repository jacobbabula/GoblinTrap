"""V4 extension, frozen from the v3 engine to preserve legacy reproduction.
...
CALIBRATION_SCENARIOS = ("fast", "switcher", "slow", "avoider", "benign_build", "benign_admin", "benign_drill")
SHIFT_SCENARIOS = ("patient", "cycler", "benign_burst", "benign_recovery")
SCENARIOS = CALIBRATION_SCENARIOS + SHIFT_SCENARIOS
...
class Config:
    ...
    isolation_coverage: float = 1.
    isolation_reliability: float = 1.
    collateral_calls_per_step: float = 0.
    background_empty_probability: float = 0.
    recognition_probability: float = 1.
    recognition_patience: int = 2
    bait_avoidance: float = 0.
    adaptive_avoidance: float = 0.

    def __post_init__(self):
        for name in ("canary_probability", "decoy_probability", "telemetry_loss",
                     "isolation_coverage", "isolation_reliability", "background_empty_probability",
                     "recognition_probability", "bait_avoidance", "adaptive_avoidance"):
        ...
        if not math.isfinite(self.collateral_calls_per_step) or self.collateral_calls_per_step < 0:
            raise ValueError("collateral_calls_per_step must be finite and nonnegative")
        if type(self.recognition_patience) is not int or self.recognition_patience < 1:
            raise ValueError("recognition_patience must be a positive integer")
...
class Agent:
    ...
    recognition_patience: int = 2
    bait_suspected: bool = False

    def choose(self) -> str:
        self.step += 1
        if self.scenario == "benign_burst":
            return ("recon", "audit", "audit", "health", "work", "wait")[(self.step-1) % 6]
        if self.scenario == "benign_recovery":
            return ("acquire", "health", "audit", "work", "health", "wait")[(self.step-1) % 6]
        if self.scenario == "patient" and self.step % 4 != 0:
            return "wait" if self.step % 4 in (1, 2) else "recon"
        ...
        if self.token == "none":
            return "acquire"
        if self.scenario in ("switcher", "cycler") and not self.persisted and not self.persistence_failed:
            return "persist"
        if not self.exfil_done:
            return "exfil"
        if not self.persisted and not self.persistence_failed:
            return "persist"
        if self.scenario in ("switcher", "cycler") and self.step % (2 if self.scenario == "cycler" else 3) == 0:
            self.channel = (self.channel + 1) % 3
            return "switch"
        return "pivot" if self.step % 2 == 0 else "exfil"

    def feedback(self, action: str, observation: str, *, recognizes_empty: bool = True):
        ...
        if observation == "empty" and recognizes_empty:
            self.empty_responses += 1
            if self.empty_responses >= self.recognition_patience:
                self.bait_suspected = True
...
def trial(scenario: str, policy: str, seed: int, config: Config = Config(),
    ...
    agent, world = Agent(scenario, recognition_patience=config.recognition_patience), World()
    ...
    for step in range(1, config.horizon + 1):
        ...
        attraction = 0. if scenario == "avoider" else 1. - config.bait_avoidance
        if agent.bait_suspected:
            attraction *= 1. - config.adaptive_avoidance
        ...
        if due == step and responded is None:
            # Revocation still closes entry/token access. Isolation may fail to
            # cover or enforce on the persistent route; draws are per episode.
            effective = response
            if response == "isolate":
                covered = draw(scenario, seed, 0, "isolation_coverage") < config.isolation_coverage
                enforced = draw(scenario, seed, 0, "isolation_reliability") < config.isolation_reliability
                if not (covered and enforced):
                    effective = "revoke"
            world.intervene(effective)
            responded = step
        if world.contained() and contained is None:
            contained = step
        observation = {"completed": "success", "decoy": "empty", "denied": "denied", "local": "local"}[outcome]
        # Empty output can now occur after a successful real operation. It
        # changes feedback only, not the operation's actual harm or tripwire.
        if (outcome == "completed" and action in ("exfil", "pivot", "audit")
                and draw(scenario, seed, step, "background_empty") < config.background_empty_probability):
            observation = "empty"
        ...
        agent.feedback(action, observation, recognizes_empty=(
            draw(scenario, seed, step, "recognition") < config.recognition_probability))
    ...
    # Additional affected workload, separate from the episode's own calls.
    # Charged on every isolation attempt, even if route enforcement failed.
    # Starts after the response action; no within-horizon charge at final step.
    collateral = (config.collateral_calls_per_step * (config.horizon - responded)
                  if response == "isolate" and responded is not None else 0.)
    return dict(collateral_disrupted_calls=collateral,
                benign_cost=disrupted + collateral if not malicious else collateral,
                scenario=scenario, policy=policy, seed=seed, malicious=malicious,
